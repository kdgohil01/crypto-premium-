...
import MultiLayerButton from '@/components/premium/MultiLayerButton';
...
export default function MultilayeredSecurity() {
  ...
  // If demo session flag is present, auto-redirect to Guardian Layer
  useEffect(() => {
    try {
      const flag = localStorage.getItem('demo_session');
      if (flag === 'true') {
        navigate('/multilayered-security/guardian-layer', { replace: true });
      }
    } catch {}
  }, [navigate]);
  const tools = [
          ...
          return (
            <Card key={tool.id} className="card-glow group hover:shadow-xl transition-all duration-300 border-primary/20">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="pt-4">
                  <MultiLayerButton className="w-full">
