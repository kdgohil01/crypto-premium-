    ...
    <div className="container mx-auto py-4 sm:py-6 md:py-8 px-4 sm:px-6 max-w-6xl">
      ...
      <div className="text-center mb-8 sm:mb-10 md:mb-12">
        <div className="inline-flex items-center justify-center w-16 h-16 sm:w-18 sm:h-18 md:w-20 md:h-20 rounded-full bg-gradient-secondary mb-4 sm:mb-5 md:mb-6 float-animation">
          <Eye className="h-8 w-8 sm:h-9 sm:w-9 md:h-10 md:w-10 text-white" />
        </div>
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-gradient mb-4 sm:mb-5 md:mb-6 leading-tight pb-2">Steganography Lab</h1>
        ...
        {tools.map((tool) => (
          <Card key={tool.title} className="card-glow group hover:scale-105 transition-transform duration-300 h-full flex flex-col">
            ...
            <CardContent className="flex flex-col flex-1">
              ...
              <Button asChild className={`w-full h-10 mt-auto ${tool.difficulty === 'Beginner' ? 'bg-blue-500 hover:bg-blue-600' : tool.difficulty === 'Intermediate' ? 'bg-green-500 hover:bg-green-600' : 'bg-red-500 hover:bg-red-600'}`}>
      ...
      <Card className="bg-muted/20 border-border/30 mb-8 sm:mb-10 md:mb-12">
        ...
        <CardContent>
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 max-w-lg mx-auto">
            <div className="text-center p-3 sm:p-4 rounded-lg bg-muted/10">
              <Volume2 className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 mx-auto mb-2 text-muted-foreground" />
              <h3 className="font-semibold mb-1 text-sm sm:text-base text-muted-foreground">Audio Steganography</h3>
              <p className="text-xs text-muted-foreground">Convert audio files into text data</p>
            </div>
            <div className="text-center p-3 sm:p-4 rounded-lg bg-muted/10">
              <Layers className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 mx-auto mb-2 text-muted-foreground" />
              <h3 className="font-semibold mb-1 text-sm sm:text-base text-muted-foreground">Video Steganography</h3>
      ...
      <Card className="card-glow">
        <CardHeader className="text-center">
          <CardTitle className="text-xl sm:text-2xl">How Steganography Works</CardTitle>
        ...
        <CardContent>
          <div className="grid gap-4 sm:gap-5 md:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            <div className="text-center">
              ...
              <h3 className="font-semibold mb-2 text-sm sm:text-base">Select Carrier</h3>
              <p className="text-xs sm:text-sm text-muted-foreground">Choose an innocent-looking file to hide data in</p>
            </div>
            <div className="text-center">
              <div className="inline-flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 rounded-full bg-secondary text-secondary-foreground font-bold mb-3 sm:mb-4 text-sm sm:text-base">
                2
              </div>
              <h3 className="font-semibold mb-2 text-sm sm:text-base">Prepare Message</h3>
              <p className="text-xs sm:text-sm text-muted-foreground">Convert secret text into binary format</p>
            </div>
            <div className="text-center">
              <div className="inline-flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 rounded-full bg-secondary text-secondary-foreground font-bold mb-3 sm:mb-4 text-sm sm:text-base">
                3
              </div>
              <h3 className="font-semibold mb-2 text-sm sm:text-base">Embed Data</h3>
              <p className="text-xs sm:text-sm text-muted-foreground">Replace least significant bits with message data</p>
            </div>
            <div className="text-center">
              <div className="inline-flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 rounded-full bg-secondary text-secondary-foreground font-bold mb-3 sm:mb-4 text-sm sm:text-base">
                4
              </div>
              <h3 className="font-semibold mb-2 text-sm sm:text-base">Extract Later</h3>
              <p className="text-xs sm:text-sm text-muted-foreground">Retrieve hidden message using the same algorithm</p>
            </div>
          </div>
        </CardContent>
      </Card>
