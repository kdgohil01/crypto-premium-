...
export default function TextInImage() {
  ...
  const { validateFile, maxFileSizeMB, isPremium } = useFileValidation();
              ...
              {previewUrl ? (
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Max file size: <span className="font-semibold">{maxFileSizeMB}MB</span>
                    {!isPremium && <span className="text-primary"> (Upgrade to Premium for 50MB)</span>}
                  </p>
                </div>
              )}
            </div>
